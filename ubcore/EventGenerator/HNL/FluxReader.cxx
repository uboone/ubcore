...
hnlgen::FluxReader::FluxReader(fhicl::ParameterSet const& p, CLHEP::HepRandomEngine& fRNG,
  ...
  fCache{}, fSelectedEntries{}
{
  ...
  std::cout << "hnlgen::FluxReader::FluxReader : please wait, initializing flux (can take some time) ... "<<std::endl;

  // some complicated code, to retry opening files on stashcache which can kill
  // jobs on the grid with errorcode 22 (file read/write) which is transient
  auto start = std::chrono::system_clock::now();
  while(true) {
    try {

      /* vvv this code is all that is needed, withouth error handling */
      ...
      if(e.explain_self().find("FatalRootError") != std::string::npos &&
          e.explain_self().find("osgstorage") != std::string::npos &&
          e.explain_self().find("can not be opened for reading") != std::string::npos) {
        // Get around "errorcode 22" when using stashcache
        ...
        while(true) {
          // wait two seconds, but still use cpu cycles to keep up efficiency (unlike sleep(2))
          auto now = std::chrono::system_clock::now();
          std::chrono::duration<double> diff = now-start_2;
          if(diff.count() > 2. || diff.count() < 0.) {
            break;
          }
        }
      }
    ...
    // down here, only if osgstorage exception was caught
    // (successfully broke loop if no exceptions; or rethrew non-osgstorage exceptions).
    // Loop again.
  }
}

hnlgen::FluxReader::~FluxReader() {
  delete fFluxTree;
}

double hnlgen::FluxReader::POTSeen(const double weight /* = 1. */) const {
...
void hnlgen::FluxReader::set_branch(void* obj, const char* bname) {
  fFluxTree->SetBranchAddress(bname, obj);
}

void hnlgen::FluxReader::get_next_entry() {
  if(fCache.empty()) {
    if(fUseCache) {
...
void hnlgen::FluxReader::get_current_entry() {
  if(fUseCache && fFullChainsSeen > 0) fFluxTree->GetEntry(fCurrEntry);
}

void hnlgen::FluxReader::count_pot(const char* bname, const char* tname) {
  ...
  std::cout << "hnlgen::FluxReader::count_pot : read "<<n_trees<<" files, with total pot = "<<fTotalPOTinFluxFiles<<std::endl;
}

double hnlgen::FluxReader::get_kaon(TLorentzVector& kmom, TLorentzVector& kpos, int& kpdg, int& pi_type) {
  get_next_entry();
